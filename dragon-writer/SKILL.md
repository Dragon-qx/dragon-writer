...
1. **识别模式**：
   ...
   - 查看进度 / 仪表盘：用户要求看进度、打开仪表盘、看设定完成度、人物关系（力导向图 + 角色详情面板 + 角色卡）或读章节（模式 F）。
...
6. **双层质量门禁**：
   ...
   - 第二层连续审计（四十维，见 audit-dimensions.md）+ 审-改循环（见 Chapter Quality Gate 第二层）。体裁裁剪 → 逐维报告 → 修订 → 回头重过 → 留痕 `audit-drift.md`。**体裁无关的"★ 始终激活"维度必须每章过**：维 27（敏感词）、维 32（读者期待）、维 33（备忘偏离），外加本章有空间 / 道具 / 动作描写时维 38（空间一致性）/ 维 39（道具追踪）/ 维 41（常识）必须进清单。
   - 两轮全过 → 落盘：写 `chapter-NNNN.intent.md`、`chapters/NNNN_<title>.md`、更新 `chapters/index.json`、写 `chapter_summaries.md` 行（含 **章节 delta**：本章改变了哪些事实 / 推了哪些伏笔 / 关系状态 / **道具增减** / **空间布局变化**——见 templates.md"章节 delta"）、更新 `current_state.md`（事实表 + 关系 + **道具账本** + **空间锚点**）、更新 `pending_hooks.md`（按诊断列治理规则）、新建快照 `snapshots/<NNNN>/`。
   - **不要重写仪表盘**——它运行时自会读取最新文件。
7. **事实验证**：落盘前回查 `current_state.md` 事实表——角色是否引用了 起始章 > 当前章 的事实？新出现的事实是否在事实表新增行？已推翻的事实是否标"已推翻-参见第 N 章"？落盘前再回查 **道具账本**（随身物件数量是否与本章描写一致）+ **空间锚点**（本章涉及的场景方位是否与既有锚点兼容，不兼容则先更新锚点再写）。
...
分两层执行。**初筛**即本节 9 点——不过直接改写；**深化**即四十维连续审计（`references/audit-dimensions.md`）——与初筛共同构成"初筛 + 深化"双保险，不是二选一。
...
- **★ 空间一致性**：本章涉及的场景，其方位 / 布局 / 出入口 / 人物相对位置，是否与 `current_state.md` 的"空间锚点"（旧场景）兼容，或在本章内（新场景）相邻段落间不互斥。本章描写了移动路径时必须经过中间空间（不可瞬移），视角内不能看见隔墙背面。
- **★ 道具与细节**：本章中人物携带的随身物件、衣物、外貌特征（参照 `current_state.md` 的"道具账本"与角色档案），其**数量、存在、状态**是否在相邻段落间与跨章无痕 ±1 / 凭空消失？消耗品的使用与减量是否对应？
- **★ 常识**：本章是否有物理上不可行（断臂者挥剑、水下点火）、因果上颠倒（未学剑就一剑封喉）、社会常识错乱（称谓、礼仪、制度与 `book_rules.md` 年代约束矛盾）的情节？架空设定例外须事先登记在 `book_rules.md`。
- 审计发现若未当场修复，必须落记录。

> 说明：原有的 7 点初筛空间 / 道具 / 常识覆盖不足——高频错误——故扩展为 9 点，把最容易在草稿阶段就出现的三类硬错拦在最前面，不必等深化审计才抓。
...
当用户要求看进度、打开仪表盘、查看设定完成度、人物关系或在仪表盘内阅读章节时使用。仪表盘是一份**自包含的 HTML 模板**（`references/dashboard.html`），通过 **File System Access API**（或 `webkitdirectory` 回退）在运行时读取书源文件并实时计算渲染。它**不嵌入任何数据**，永远反映最新文件，因此只需生成一次、无需每章重写。共 5 个标签页：总览、设定完成度、设定内容、人物关系、章节阅读。
...
- 授权后仪表盘读取 `book.json` / `chapters/*.md` / `story/*.md`，实时计算并渲染 5 个标签页：
  - **总览**：进度环 + 统计卡 + 当前状态 + 最近章节表 + 审计漂移；
  - **设定完成度**：4 份设定文件的逐域进度条，点开见子项完成状态；
  - **设定内容**：4 份设定文件原文 Markdown 全文渲染，可展开 / 折叠；
  - **人物关系**：左侧 `<canvas>` 力导向关系图（可拖拽节点、点击高亮）+ 右侧固定角色详情信息板（故事功能 / 欲望 / 恐惧 / 当前状态 / 弧线 / 秘密 / 全部关系）+ 角色卡网格；
  - **章节阅读**：左栏目录 + 右栏 Serif 字体 Markdown 渲染 + 上一章 / 下一章导航，支持一键导出全本 TXT。
